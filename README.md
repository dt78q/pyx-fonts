...
# Details
To be memory efficient, each font is encoded as a single tuple of hex strings (one string for each glyph) with each hex character encoding four pixels. In addition, only the active area of each glyph is encoded, through a pair of offset parameters, and unneeded characters can be simply deleted so different sized fonts can be combined provided the total number of characters is not too great. The example `pyx graf demo.py` displays almost 500 different characters on a Pico using the stock Waveshare 320&nbsp;×&nbsp;240 display driver, with its >150&nbsp;kB memory footprint. Sizes 8, 9, 11, 13, & 15 make a very useful set. Each font has an alternative modern 'a' glyph with a d-like bowl.
# Notes
